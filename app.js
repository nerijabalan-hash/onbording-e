...
const onboarding = {
  ...
  init() {
    ...
    // Pick up version from the HTML page's window variable
    ...
    // Skip the first step (library/collapsed pane) if configured
  ...
  placeDot() {
    ...
    // Version E: only show halo around the button, no dot
    if (this.currentVersion === 'E' && this.eHaloEl) {
      document.body.appendChild(this.eHaloEl);
      // Add white bg class to the halo target (thumb or button)
      const cardEl = target.closest('.asset-card');
      const haloTarget = cardEl ? (cardEl.querySelector('.asset-card-thumb') || cardEl) : target;
      haloTarget.classList.add('onboarding-e-halo-target');
      this._eHaloTarget = haloTarget;
    ...
    this._targetHoverIn = () => {
      this.showPopover();
      if (this.eHaloEl) this.eHaloEl.classList.add('halo-hidden');
    };
    this._targetHoverOut = () => {
      this.hidePopover();
      if (this.eHaloEl) this.eHaloEl.classList.remove('halo-hidden');
    };
  ...
  _startPositionLoop(target) {
    ...
    const step = ONBOARDING_STEPS[this.currentStepIndex];
    const loop = () => {
      // Version E uses only halo, no dot
      const hasDot = this.dotEl && this.dotEl.parentNode;
      const hasHalo = this.currentVersion === 'E' && this.eHaloEl && this.eHaloEl.parentNode;
      if (!hasDot && !hasHalo) return;
      const el = document.querySelector(step?.targetSelector);
      if (el) {
        if (hasDot) this._positionDotOnTarget(el, step?.dotOffset);
      ...
      this.positionRAF = setTimeout(loop, 100);
    };
    this.positionRAF = setTimeout(loop, 100);
  },
  ...
  removeDot() {
    ...
    if (this._eHaloTarget) {
      this._eHaloTarget.classList.remove('onboarding-e-halo-target');
      this._eHaloTarget = null;
    }
  ...
  positionEHalo(target) {
    ...
    // For card steps, wrap just the thumbnail
    const thumb = target.closest('.asset-card') ? target.closest('.asset-card').querySelector('.asset-card-thumb') || target : null;
    const haloTarget = thumb || target;
    ...
    // Match border-radius to target + pad offset
    const targetRadius = parseInt(getComputedStyle(haloTarget).borderRadius) || 6;
    this.eHaloEl.style.borderRadius = (targetRadius + pad) + 'px';
  ...
  completeAll() {
    ...
    // Loop: restart the flow after a short pause
    setTimeout(() => {
      this.currentStepIndex = window.ONBOARDING_SKIP_FIRST_STEP ? 1 : 0;
  ...
  onRender() {
    if (!this.popoverEl) return; // init() hasn't run yet
    if (this.isComplete) return;
...
// --- Init ---
try { updateSortIcon(); } catch(e) { console.error('updateSortIcon:', e); }
try { renderSlideThumbnails(); } catch(e) { console.error('renderSlideThumbnails:', e); }
try { render(); } catch(e) { console.error('render:', e); }
try { setupSlideDropTarget(); } catch(e) { console.error('setupSlideDropTarget:', e); }

// Initialize onboarding after initial render
try { onboarding.init(); } catch(e) { console.error('onboarding.init:', e); }
